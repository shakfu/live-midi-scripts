...
# Embedded file name: /MIDI Remote Scripts/pushbase/push_base.py
...
class PushBase(ControlSurface):
    ...
    def process_midi_bytes(self, midi_bytes, midi_processor):
        recipient = (midi.is_sysex(midi_bytes) or self.get_recipient_for_nonsysex_midi_message)(midi_bytes)
        if isinstance(recipient, ButtonElement) and midi.extract_value(midi_bytes) != 0:
