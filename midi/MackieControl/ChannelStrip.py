...
# Embedded file name: /MIDI Remote Scripts/MackieControl/ChannelStrip.py
...
class ChannelStrip(MackieControlComponent):
    ...
    def handle_channel_strip_switch_ids(self, sw_id, value):
        ...
        if sw_id in range(SID_SOLO_BASE, SID_SOLO_BASE + NUM_CHANNEL_STRIPS):
            ...
            if sw_id - SID_VPOD_PUSH_BASE is self._ChannelStrip__strip_index:
                if value == BUTTON_PRESSED and self._ChannelStrip__channel_strip_controller != None:
        ...
        elif sw_id in fader_touch_switch_ids and sw_id - SID_FADER_TOUCH_SENSE_BASE is self._ChannelStrip__strip_index:
    ...
    def __add_listeners(self):
        if self._ChannelStrip__assigned_track and self._ChannelStrip__assigned_track in self.song().tracks:
