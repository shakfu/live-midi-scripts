...
# Embedded file name: /MIDI Remote Scripts/ableton/v2/control_surface/components/channel_strip.py
...
class ChannelStripComponent(Component):
    ...
    def update_solo_stateParse error at or near `COME_FROM' instruction at offset 44_1
                        ...
                        for track in self.song.tracks:
                            if track.can_be_armed:
                                if not not track == self._track:
                                    if not respect_multi_selection or track.is_part_of_selection:
                                        track.arm = new_value
                                    if arm_exclusive:
                                        if track.arm:
                                            track.arm = False
    ...
    def _on_mute_changed(self):
        if self.is_enabled():
            if not self._mute_button is not None or liveobj_valid(self._track) or self.empty_color is None:
    ...
    def _on_solo_changed(self):
        if self.is_enabled():
            if not self._solo_button is not None or liveobj_valid(self._track) or self.empty_color is None:
    ...
    def _on_arm_changed(self):
        if self.is_enabled():
            if not self._arm_button is not None or liveobj_valid(self._track) or self.empty_color is None:
