...
# Embedded file name: /MIDI Remote Scripts/ableton/v3/control_surface/components/device.py
...
class DeviceComponent(ParameterProvider, Component, Renderable):
    ...
    device_fold_button = ToggleButtonControl(color="Device.FoldOff",
      on_color="Device.FoldOn",
      enabled=False)
    ...
    def _set_device(self, device):
        ...
        self._DeviceComponent__on_is_showing_chain_devices_changed.subject = device.view if is_rack else None
        self.device_fold_button.enabled = is_rack
        if is_rack:
            self._DeviceComponent__on_is_showing_chain_devices_changed()
        self.notify_device()
    ...
    def _get_provided_parameters(self):
        ...
        if self._banking_info.num_simultaneous_banks > 1:
            return deduplicate_parameters(parameter_infos)
