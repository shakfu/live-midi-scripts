...
# Embedded file name: /MIDI Remote Scripts/ableton/v3/control_surface/components/note_editor.py
# Compiled at: 2024-08-28 00:56:44
# Size of source mod 2**32: 28662 bytes
...
class NoteEditorComponent(Component, Renderable):
    ...
    def _get_property_range_string(self, property_name, value_range_fn, str_fmt="{:.0f}".format):
        ...
        if property_name in property_ranges:
            ...
            if p_min == p_max:
                return max_str
            return "{} - {}".format(str_fmt(p_min), max_str)
        return "No Notes"

    def _get_step_offset_range_string(self, property_name):
        return self._get_property_range_string(property_name,
          (lambda value_range: (
         step_offset_percentage(self.step_length, value_range[0]),
         step_offset_percentage(self.step_length, value_range[1]))),
          str_fmt=("{}%".format))
    ...
    def _modify_note_property(self, note_property, value):
        if self.is_enabled():
            ...
            if self._active_steps and self._has_clip():
                if self._can_edit():
        ...
        if self._has_clip():
            ...
            if self._clipboard.has_content:
                ...
                self._clipboard.extend_buffer((
                 region_start, region_start + self.step_length))
