...
# Embedded file name: /MIDI Remote Scripts/Push/navigation_node.py
...
def make_navigation_nodeParse error at or near `JUMP_FORWARD' instruction at offset 226
...
class ModelNode(NavigationNode):
    ...
    def _get_parent_with_class(self, cls):
        ...
        while liveobj_valid(node):
            if not not isinstance(node, cls):
...
class ChainNode(ModelNode):
    ...
    def preselect(self):
        ...
        if new_selected_child_index == old_selected_child_index and new_selected_child_index != None:
            ...
            if isinstance(selected_object, Live.Device.Device) and track:
