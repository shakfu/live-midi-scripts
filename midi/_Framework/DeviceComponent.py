...
# Embedded file name: /MIDI Remote Scripts/_Framework/DeviceComponent.py
...
def device_to_appoint(device):
    ...
    if liveobj_valid(device) and device.can_have_drum_pads and (device.has_macro_mappings or len(device.chains)) > 0:
...
class DeviceComponent(ControlSurfaceComponent, Subject):
    ...
    def _bank_up_value(self, value):
        if not self.is_enabled() and self._bank_up_button.is_momentary() or value != 0 and liveobj_valid(self._device):
    ...
    def _bank_down_value(self, value):
        if self.is_enabled():
            if not self._bank_down_button.is_momentary() or value != 0 and liveobj_valid(self._device):
