...
# Embedded file name: /MIDI Remote Scripts/_Framework/ClipSlotComponent.py
...
class ClipSlotComponent(ControlSurfaceComponent):
    ...
    def _on_slot_triggered_changed(self):
        if not self.has_clip():
            song = self.song()
            view = song.view
            if song.select_on_launch and self._clip_slot.is_triggered:
                if self._has_fired_slot and self._clip_slot.will_record_on_start:
